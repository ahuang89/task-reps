const dmPsych = (function() {
  ...
  */
  ...
  obj.MakeTileGame = function(hex, tileHit, tileMiss, roundLength, gameType, nTrials, pM, blockName, roundNum) {
    ...
    const tokens_html = `<div class="outcome-container-dm">
                          <div class="header-win" style="color:${hex}">{header}</div>
                          <div class="token-text-win" style="color:${hex}">${winFeedback}</div>
                        </div>`;

    const tokens_bonus_html = `<div class="outcome-container-dm">
                                <div class="header-win" style="color:${hex}">{header}</div>
                                <div class="token-text-win" style="color:${hex}">${winFeedback}</div>
                                <div class="bonus-text">+5 Bonus</div>
                              </div>`;

    const tokens_loss_html = `<div class="outcome-container-dm">
                                <div class="header-win" style="color:${hex}">{header}</div>
                                <div class="token-text-win" style="color:${hex}">${winFeedback}</div>
                                <div class="penalty-text">-5 Loss</div>
                              </div>`;

    const noTokens_html = `<div class="outcome-container-dm">
                            <div class="header-lose" style="color:grey">{header}</div>
                            <div class="token-text-lose">${lossFeedback}</div>
                          </div>`;

    const noTokens_bonus_html = `<div class="outcome-container-dm">
                                  <div class="header-lose" style="color:grey">{header}</div>
                                  <div class="token-text-lose">${lossFeedback}</div>
                                  <div class="bonus-text">+5 Bonus</div>
                                </div>`;

    const noTokens_loss_html = `<div class="outcome-container-dm">
                                  <div class="header-lose" style="color:grey">{header}</div>
                                  <div class="token-text-lose">${lossFeedback}</div>
                                  <div class="penalty-text">-5 Loss</div>
                                </div>`;

    const iti_html = `<div class="outcome-container-dm">
                                <div class="header">{header}</div>
                              </div>`;

    const probe_html = `<div class="outcome-container-dm">
                          <div class="header">{header}</div>
                          <div class="box" style="background-color:gray"></div>
                        </div>`;

    const warning_html = `<div class="outcome-container-dm">
                            <div class="header">{header}</div>
      ...
      stimulus: () => {
        if (gameType == 'bern' | gameType == 'bern-mod-PE') {
          if (tooSlow) {
            ...
            message = noTokens_html;
            if (blockName !== "practice") {
              totalTokens += bernTokens_miss;
            };            
            round++;
        ...
        if (gameType == '1inN') {
          if (tooSlow && losses < 5) {
            ...
            losses = 0;
            let feedbackType = lossArray.pop();
            message = noTokens_html.replace("{header}", `Attempts remaining: 0`);
            if (blockName !== "practice") {
              totalTokens += 0;
            };
            round++;
            ...
            losses = 0;
            let feedbackType = winArray.pop();
            message = tokens_html.replace("{header}", '');
            if (blockName !== "practice") {
              totalTokens += 10;
            };            
            round++;
        ...
        if (gameType == 'strk' | gameType == "strk-mod") {
          if (tooSlow && streak > 0) {
            ...
            if (blockName !== "practice") {
              totalTokens += (finalStreak*multiplier);
            };    
